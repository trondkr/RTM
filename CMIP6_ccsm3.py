...
class CMIP6_CCSM3:
    ...
    def direct_and_diffuse_albedo_from_snow_and_ice(
        ...
        albo_dr = np.where(ice_thickness > 0.0001, albo_i, albo_dr)
        ...
        albo_dr = np.where(
            snow_thickness > snowpatch, albsnowv * snow_concentration, albo_dr
        )
    ...
    def calc_ice_attenuation(
        ...
        """
        logging.debug(
            f"[CMIP6_ccsm3] calc_ice_attenuation started for spectrum {spectrum}"
        )
