...
class MyCMIP6cesm3(unittest.TestCase):
    ...
    def test_albedo_correct_when_sea_air_temp_is_very_cold_and_full_ice_snow_cover(
        ...
        np.testing.assert_almost_equal(
            albo_dr, np.ones(np.shape(self.snow_thickness)) * 0.962, decimal=2
        )
    ...
    def test_albedo_correct_when_sea_air_temp_is_very_warm_and_full_ice_no_snow_cover(
        ...
        ice_thickness = np.ones(np.shape(self.snow_thickness)) *0.5
        sea_ice_concentration = np.ones(np.shape(self.snow_thickness))
        snow_thickness = np.zeros(np.shape(self.snow_thickness))
        snow_concentration = np.zeros(np.shape(self.sea_ice_concentration))
        ocean_albedo = np.ones(np.shape(snow_thickness)) * 0.06
        ...
        # Make sure the effect of pond melting is absolutely none
        air_temp = np.ones(np.shape(self.snow_thickness)) +6.
        albo_dr = self.CMIP6_cesm3.direct_and_diffuse_albedo_from_snow_and_ice(
            ocean_albedo,
            snow_concentration,
            snow_thickness,
        ...
        np.testing.assert_almost_equal(
            albo_dr, np.ones(np.shape(self.snow_thickness)) * 0.69, decimal=2
        )
    
    def test_albedo_correct_when_sea_air_temp_is_very_warm_and_full_ice_and_snow_cover(
        self,
    ):
        # https://csdms.colorado.edu/w/images/CICE_documentation_and_software_user's_manual.pdf
        # Very thick ice, but very warm weather creating melt ponds that drastically decreases
        ...
        ice_thickness = np.ones(np.shape(self.snow_thickness)) * 0.5
        sea_ice_concentration = np.ones(np.shape(self.snow_thickness))
        snow_thickness = np.ones(np.shape(self.snow_thickness))+10
        snow_concentration = np.ones(np.shape(self.sea_ice_concentration))
        ocean_albedo = np.ones(np.shape(snow_thickness)) * 0.06
    ...
    def test_albedo_correct_when_sea_air_temp_is_warm_and_full_ice_no_snow_cover(
        self,
    ):
        # https://csdms.colorado.edu/w/images/CICE_documentation_and_software_user's_manual.pdf
        # Very thick ice, but very warm weather creating melt ponds that drastically decreases
        ...
        # Make sure the effect of pond melting is absolutely none
        air_temp = np.zeros(np.shape(self.snow_thickness)) - 10
        albo_dr = self.CMIP6_cesm3.direct_and_diffuse_albedo_from_snow_and_ice(
            ocean_albedo,
            snow_concentration,
            snow_thickness,
    ...
    def test_albedo_correct_when_sea_air_temp_is_warm_and_no_snow_or_ice_cover(
        self,
    ):
        # https://csdms.colorado.edu/w/images/CICE_documentation_and_software_user's_manual.pdf
        # Very thick ice, but very warm weather creating melt ponds that drastically decreases
        ...
        np.testing.assert_almost_equal(
            albo_dr, np.ones(np.shape(self.snow_thickness)) * 0.06, decimal=2
        )
    ...
    def test_albedo_correct_when_very_thin_ice_no_snow_cover_converge_to_ocean_osa(
        ...
        sea_ice_concentration = np.ones(np.shape(self.snow_thickness))
        snow_thickness = np.ones(np.shape(self.snow_thickness))
        snow_concentration = np.zeros(np.shape(self.sea_ice_concentration))
        ocean_albedo = np.ones(np.shape(snow_thickness)) * 0.06
    ...
    def test_albedo_correct_when_thin_ice_but_more_than_10cm_no_snow_cover(
        ...
        # Make sure the effect of pond melting is absolutely none
        air_temp = np.ones(np.shape(self.snow_thickness))-20
        albo_dr = self.CMIP6_cesm3.direct_and_diffuse_albedo_from_snow_and_ice(
            self.ocean_albedo,
            snow_concentration,
            snow_thickness,
    ...
    def test_calculate_diffuse_albedo_per_grid_point_all_snow(self):
        ...
        np.testing.assert_almost_equal(
            albedo, np.ones(np.shape(self.snow_thickness)) * 0.98, decimal=2
        )
    ...
    def test_calculate_diffuse_albedo_per_grid_point_all_ice_no_snow(self):
        ...
        np.testing.assert_almost_equal(
            albedo, np.ones(np.shape(self.snow_thickness)) * 0.78, decimal=2
        )
    ...
    def test_calculate_diffuse_albedo_per_grid_point_all_ice_except_one_grid_cell(self):
        ...
        result_should_be = np.ones(np.shape(self.snow_thickness)) * 0.78
