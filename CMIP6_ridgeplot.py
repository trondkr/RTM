...
class CMIP6_ridgeplot:
    ...
    @classmethod
    def calculate_climatology(cls, ds: xr.Dataset, infile: str, cumulative: bool = True,
                              seasonal: bool = False):
        if cumulative:
            if seasonal:
            ...
            if seasonal:
                climatology = ds.groupby("time.season").mean("time", keep_attrs=True, skipna=True)
            else:
                climatology = ds.groupby("time.month").mean("time", keep_attrs=True, skipna=True)
    ...
    def create_ridgeplot(
            cls, var_name, df: pd.DataFrame, ds: xr.Dataset, outfile: str, labels: str, seasonal: bool = True,
        ...
        colors = [color1, color2, color3, color4, color5, color6,color1, color2, color3, color4, color5, color6,color1, color2, color3, color4, color5, color6,color1, color2, color3, color4, color5, color6,color1, color2, color3, color4, color5, color6,color1, color2, color3, color4, color5, color6]
        ...
        if seasonal:
            panels = 4
            g1 = sns.FacetGrid(
                df, row="season", hue="season", aspect=15, height=1.2
            )
        else:
            panels = 12
            g1 = sns.FacetGrid(
                df, row="month", hue="month", aspect=15, height=1.2
            )
        ...
        for i, label in enumerate(labels):
            ...
            g1.map(sns.kdeplot, var_name, clip_on=False, multiple="layer", cumulative=cumulative_plot, color=colors[i],
                   lw=1,
                   bw_adjust=0.5,
                   log_scale=log, legend=False, clip=clip)
    ...
    @classmethod
    def heatmap_of_change(self, max_df_dict: [], var_name: str, labels: [], panels: [], LME: str, scenario: str):
      ...
      #      max_df_dict[label]=(max_df_dict[label]-max_df_dict[label].mean())/max_df_dict[label].std()
        ...
        with sns.axes_style("white"):
            sns.heatmap(clim, square=True,
                        ...
                        annot=annot_labels,  fmt=fmt, linewidth=0.5, annot_kws={'fontsize': fontsize})
    ...
    def ridgeplot(
      ...
      #  end_times = [datetime(2030, 1, 1), datetime(2099, 12, 1)]
        ...
        for start, end in zip(start_times, end_times):
            ...
            labels.append(f"{start.year}-{end.year}", )
        ...
        cls.create_ridgeplot(var_name, df_combined, ds, outfile, labels, seasonal, cumulative)
...
if __name__ == "__main__":
    ...
    CMIP6_ridgeplot.ridgeplot(
        "thetao_mean", infile, outfile, glorys=False, depth_threshold=9000, cumulative=cumulative, seasonal=seasonal,
    )
